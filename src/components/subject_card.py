...
def subject_card(name, code, section, stats=None, footer_callback=None):
    ...
    if stats:
        html+= """
        <div style="display:flex; gap:8px; flex-wrap:wrap;color:black ">
        """
        for icon, label, value in stats:
            html+= f'<div style="background: pink; padding:5px 12px; border-radius:12px; font-size:0.9rem">{icon} <b>{value}</b> {label} </div>'
